...
public class RelationshipRepositoryTest : AbstractRepositoryTest<RelationshipRepository, Relationship, SaveRelationship>
{
    ...
    {
        yield return new TestCaseData(new Relationship(4, RelationshipType.ParentOf, 4, "Silco", 2, "Powder"),
            RelationshipType.ChildOf);
    }

    [Test, TestCaseSource(nameof(CreatePseudoSymmetricalRelationships))]
    public void FindByPersonIdFromPseudoSymmetricalRelationships(Relationship relationship,
        RelationshipType oppositeType)
    {
        var result = ClassUnderTest.FindByPersonId(relationship.FromId);
    ...
    {
        return new Relationship(relationship.Id, oppositeType, relationship.ToId, relationship.ToName,
