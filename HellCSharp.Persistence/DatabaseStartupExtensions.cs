...
namespace HellCSharp.Persistence
{
    public static class DatabaseStartupExtensions
    {
        ...
        public static void AddDatabaseService(this IServiceCollection services, String connectionString)
        {
            ...
            {
                services.AddDbContext<Database.Database>(options => options.UseInMemoryDatabase("Filename=TestDatabase.db"));
            }
            else
            {
                // TODO 1) figure out how to start app in production, 2) figure out how to define actual database 3) use both here
                throw new Exception("Actual database connection is not implemented yet! (defaultConnection=" + connectionString + ")");
            }
